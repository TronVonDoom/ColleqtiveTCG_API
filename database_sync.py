...
# Disable SSL warnings (for VPN/proxy environments)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                response = requests.get(
                    ...
                    params=params,
                    timeout=180,  # 3 minutes for very slow VPN connections
                    verify=False  # Disable SSL verification for VPN/proxy
                )
                ...
                logger.warning(f"Request timeout on attempt {attempt + 1}/{MAX_RETRIES}: {e}")
                if attempt < MAX_RETRIES - 1:
                    wait_time = RETRY_DELAY * (2 ** attempt)
